...
-- ================================================================
-- ⚔️ ULTIMATE STEAL AN EGG - FULL FEATURE SET + PRO CYBER UI + ESP
-- ================================================================

-- Safe Execution Wrapper for GUI
task.spawn(function()
    local success, err = pcall(function()
        ...
        local function GetBatTool()
            ...
            if bp then
                for _, t in ipairs(bp:GetChildren()) do
                    if t:IsA("Tool") and (t.Name:lower():find("bat") or t.Name:lower():find("sword") or t.Name:lower():find("weapon")) then
                        return t
                    end
                end
            end
        ...
        local function EquipBat()
            ...
            if hum and tool and tool.Parent ~= GetChar() then
                hum:EquipTool(tool)
            end
        end
        ...
        local function AttackWithBat()
            pcall(function()
                local tool = GetBatTool()
                if tool and tool.Parent == GetChar() then
                    tool:Activate()
                end
            end)
        end
        ...
        local function PlaceEggAtPlot()
            ...
            if hrp then
                SafeWalkTo(plotPos, 5.0, 5, State.SprintSpeed)
                InstantTriggerSteal()
                task.wait(0.2)
            end
        end

        -- Animation Freeze Loop
        task.spawn(function()
            while true do
                task.wait(0.2)
                if State.FreezeAnimations then
                    pcall(function()
                        local hum = GetHumanoid()
                        if hum then
                            for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                                if track.Animation and track.Animation.AnimationId then
                                    ...
                                    if id:find("walk") or id:find("run") or id:find("move") then
                                        track:Stop()
                                    end
                                end
                            end
        ...
        -- Main Automation Controller Loop
        local function MainControllerLoop()
            task.spawn(function()
                while State.Running do
                    local hum = GetHumanoid()
                    ...
                    if State.CurrentMode == "FARM" and stageData then
                        State.Status = "🏃 RUSHING TO " .. stageData.name
                        SafeWalkTo(stageData.pos, 6.0, 8, State.SprintSpeed)
                        ...
                        State.Status = "🏃 EVADING TO SPAWN!"
                        SafeWalkTo(GameData.StartSpawn, 5.0, 10, State.SprintSpeed)
        ...
        ToggleBtn.Position = UDim2.new(0, 25, 0, 160)
        ...
        TitleLbl.Text = "Steal An Egg - Pro Master"
        ...
        local categories = {"Steal & Farm", "PvP Combat", "ESP & Visuals", "Pets Wiki", "Settings"}
        local catButtons = {}
        local catPages = {}
        ...
        CreateToggleRow(farmPage, "Auto Treadmill (AFK Base)", false, function(v)
        ...
        CreateToggleRow(farmPage, "Freeze Player Animations", false, function(v)
        ...
        stageRow.Position = UDim2.new(0, 15, 0, 135)
        ...
        pTitle.Text = "Pets Database & Stats"
        ...
        pInfo.Size = UDim2.new(1, -30, 0, 100)
        pInfo.Position = UDim2.new(0, 15, 0, 50)
        pInfo.BackgroundTransparency = 1
        pInfo.Text = "• Forest Pets: Chicken, Dog, Bird ($1 - $8/s)\n• Desert Pets: Scorpion, Camel ($15 - $45/s)\n• Volcano Pets: Magma Imp ($120/s)\n• Void Pets: Void Dragon (Secret, $5,000/s)"
        ...
        print("Steal An Egg Full Pro Master UI Loaded Successfully!")
    end)
    if not success then
        warn("StealAnEgg Full Script Error:", err)
    end
end)
